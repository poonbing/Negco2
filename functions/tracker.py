...
class Tracker:
    ...
    def get_active_trackers(self, user):
        try:
            ...
            data = {'Air Conditioning': ['Aircon', 10, None, None], 'Shower': [
                'Shower', 10, None, None], 'Room Lights': ['Lights', 10, None, None]}
    ...
    def stop_active_tracker(self, user, tracker_id):
        ...
        for i in tracker_tray:
            ...
            if i == tracker_id:
                rate = tracker_tray[i][1]
                tracker_tray[i] = [item, rate, None, None]
    ...
    def update_tracker_session(self, item, rate, user, key):
        if key == 'None':
            ...
            data = {'user': user, 'item': item,  'rate': rate,
                    'time_start': current_time,  'time_end': None}
