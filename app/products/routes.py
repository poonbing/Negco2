    ...
    if request.method == "POST":
        ...
        if offer > 0:
            ...
            current_app.logger.info(
                "Added product ( %s ) with offered price ( %s) from %s for %s",
                name,
                round(offered_price, 2),
                request.remote_addr,
                request.path,
            )
...
def viewProduct():
    current_app.logger.info(
        "Processing request to view products from %s for %s",
        request.remote_addr,
        request.path,
    )
...
def updateProduct(id):
    ...
    if request.method == "POST":
        ...
        try:
            db.session.commit()
            current_app.logger.info(
                "Product ( %s ) updated from %s for %s",
                form.data.name,
                request.remote_addr,
                request.path,
            )
            ...
            current_app.logger.info(
                "Product update rejected from %s for %s",
                request.remote_addr,
                request.path,
            )
...
def deleteProduct(id):
    ...
    try:
        ...
        db.session.commit()
        current_app.logger.info(
            "Product ( %s ) deleted from %s for %s",
            product_to_delete.name,
            request.remote_addr,
            request.path,
        )
        ...
        current_app.logger.info(
            "Product delete failed from %s for %s", request.remote_addr, request.path
        )
        return render_template(
...
def productPage(id):
    ...
    current_app.logger.info(
        "Processing request to view product page %s from %s for %s",
...
def allProducts():
    ...
    current_app.logger.info(
        "Processing request to view all products from %s for %s",
        request.remote_addr,
        request.path,
    )
...
def filterProducts(category):
    ...
    current_app.logger.info(
        "Filtering products from %s for %s", request.remote_addr, request.path
    )
    return render_template(
...
def add_to_cart(product_id):
    ...
    db.session.commit()
    current_app.logger.info(
        "Added Product ( %s ) to cart from %s for %s",
        product.name,
        request.remote_addr,
        request.path,
    )
...
def view_cart():
    ...
    current_app.logger.info(
        "Processing request to view cart from %s for %s",
        request.remote_addr,
        request.path,
    )
    return render_template(
...
def update_quantity(product_id):
    ...
    if cart_item:
        ...
        db.session.commit()
    current_app.logger.info(
        "Updated product ( %s ) quanity ( %s ) from %s for %s",
        cart_item.name,
        cart_item.quantity,
        request.remote_addr,
        request.path,
...
def remove_from_cart(product_id):
    ...
    if cart_item:
        ...
        db.session.commit()
        current_app.logger.info(
            "Cart item ( %s ) removed from %s for %s",
            cart_item.name,
            request.remote_addr,
            request.path,
        )
